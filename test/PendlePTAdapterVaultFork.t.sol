...
contract PendlePTAdapterVaultForkTest is Test {
    ...
    function test_vault_partialWithdraw_preMaturity() public onlyFork {
        ...
        // Hardened core `_exitRealize` is best-effort and pays `payout <= want`, truncating
        // by <=1 unit (6-dec USDC) vs the old strict-exact delivery. Tolerate the 1-unit floor.
        assertApproxEqAbs(IERC20(USDC).balanceOf(user) - userBefore, want, 1, "partial withdraw off by >1 unit");
        assertLe(IERC20(USDC).balanceOf(user) - userBefore, want, "partial over-delivered");

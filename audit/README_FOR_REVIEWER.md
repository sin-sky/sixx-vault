...
- 会計コア mutation：
  - ⚠️ **現行 headline = 凍結 `960b707` のフル 1,090-mutant run＝score 94.6%（killed 1031 / survived 59, seed=0）。** 生存 59 はいずれも**設計上の等価変異**（`_totalDebt`/`performanceFee` 等）＋**低 severity の negative/telemetry テスト欠落**で、安全性 invariant は非破壊。

...
## 確定値（フル1090 再実行の結果）— 看板は VALIDATED（訂正不要）

硬化ハーネス（**pre-flight canary が SOUND を確認**: 未変異 src で 280 tests pass）による
`MUTATION_N=2000`（=1090 全件・ダウンサンプル無し）/`seed=0`/現 tip の再実行結果:
...
→ **看板 stat「94.6% / 1090 / killed 1031 / survived 59」を厳密に一致再現。看板は正当（correction 不要）。**
- 分類 invocation は `forge test --no-match-contract Fork -q`（`MUTATION_MATCH` 未設定＝`'*'` 異常終了クラス非該当）。
- **as-run ログを `audit/mutation-full-1090-evidence-2026-07-12.md` に保全**（単一ソース・生存 59 件を全記録）。
- 本セッションで追加した 8 テスト後も score は 1031/59 で不変（追加テストは Ethena/Registry 対象で SIXXVault コア mutant に非影響＝整合）。

### 生存 59 件の性質（看板の記述と一致）
看板は「生存 59 = 設計上の**等価変異** ＋ 低 severity の **negative/telemetry テスト欠落**、安全性 invariant は非破壊」と述べる。
再実行の 59 件はこの記述どおり:
- **等価変異**: `performanceFee`（accrual 未使用）・`managementFee==0`/`feeShares>0`/`elapsed==0` の fee 短絡・
  `_totalDebt` bookkeeping・timestamp telemetry（`_lastReport`/`_lastHarvestTimestamp` の =0/=1）・
  二重ガード済 preview（`maxDeposit`/`maxWithdraw` の shutdown/pause 分岐は `_deposit`/`_withdraw` 側の revert が本防御）。
- **低 severity test 欠落**: force-detach/recall の best-effort bookkeeping（#463/#470/#493 等）・timelock catch-revert 経路（#1018）。
  いずれも **pre-existing（SIXXVault コア、最近の差分非該当）** かつ会計コア安全性 invariant
  （deposit 無価値創出なし・redeem≤deposit＝Halmos/INV で別途 green）を破らない。

> **S0-1 判定**: 看板は偽 kill 由来ではなく、健全な invocation の run と**厳密一致で再現＝証明**された。
> **訂正は不要**。README_FOR_REVIEWER / SCOPE の数字は正しい。差分は evidence ファイルへのポインタ追記のみ。

---

## 完了（S0 クローズ）

- S0-1: 看板 stat を硬化ハーネスで**厳密再現＝証明**（as-run ログを evidence 保全）。訂正不要。
- S0-2: clean-tree guard（Stage 0a）＋ handoff の Slither/Aderyn を clean tree で再現確認。
- S0-3: measurement 回帰（Stage 0c, A–F 全 green）＋ canary で偽 kill/偽 survive の両クラスを事前検出。
- `contract-audit.sh` 全ゲート clean tree で OVERALL PASS。凍結 src 無改変。

**→ 測定器の修理は完了。Round 8 開始の前提（S0 完了）を満たした。**

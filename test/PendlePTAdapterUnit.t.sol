...
contract PendlePTAdapterUnitTest is Test {
    ...
    function test_ctor_twapBelow15min_reverts() public {
        // Part B P3 (OR2): min 15-min (900s) TWAP window, restored in the audit-scope
        // aggregate. twap=0 and twap=899 are both rejected with the 15-min message.
        vm.expectRevert(bytes("ADAPTER: twap < 15min"));
        _construct(address(usdc), address(market), address(router), address(oracle), address(swapper), 0, address(this), governance);

        vm.expectRevert(bytes("ADAPTER: twap < 15min"));
        _construct(address(usdc), address(market), address(router), address(oracle), address(swapper), 899, address(this), governance);
    }

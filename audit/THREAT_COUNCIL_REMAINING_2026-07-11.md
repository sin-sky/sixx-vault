...
- 実挙動として拾った **LOW/informational は3件**（いずれも資金喪失リスクなし・是正は任意）：
  1. **RD5**：zero-share になる極小 deposit が資産（dust ≤ 入金額）を取り 0 share を mint（OZ v5 ERC-4626 標準挙動・自己負担 dust・第三者搾取不能・非 insolvency）。
  2. **AC8-obs**：`setPerformanceFee` / `setManagementFee` と Registry の gov 移転が event 未発火（可観測性）。
  3. **OR2/⑤ 継続**：Pendle `twapDuration` に in-contract の下限がない（immutable＋deploy 時 oracle-readiness で緩和）。
- 上記＋既存の carry-over（`performanceFee` dead-code・本番 Timelock/Safe 強制）を **`REMEDIATION_PROPOSALS.md`** に Part B として起票（凍結コードには入れない・SHIN 承認待ち）。
- **新規 PoC 28本を green で在 repo**：`test/ThreatCouncilRemaining.t.sol`。非フォーク全 182 本 green 維持。
...
| ③ | `test_RD5_zeroShareDeposit_isBoundedSelfInflictedDust` | zero-share 入金は自己負担 dust に限定・他者無害・非 insolvency（LOW 起票） |
...
| AC8 | timelock＋event 可観測性 | **一部/要提案(LOW)** | timelock は gov 層（外部 TimelockController・PoC `TimelockGovernance.t.sol`）。event＝大半発火（AdapterUpdated/LockPeriodUpdated/EmergencyShutdown/Governance*/GuardianChanged/FeeCollected）。**GAP**＝`setPerformanceFee`/`setManagementFee` と registry gov 移転が event 未発火。→ `REMEDIATION_PROPOSALS` AC8。 |
...
| RD5 | zero-share mint | **要提案(LOW)** | OZ v5 は `shares>0` gard 無し＝極小入金が dust を取り 0 share。PoC：**自己負担 dust ≤ 入金額・第三者搾取不能・他者無害・非 insolvency**。任意 `require(shares>0)`→ `REMEDIATION_PROPOSALS` RD5。 |
...
| OR2 | Pendle 満期前 MTM | **安全確認済(将来注記)** | AMM spot は execution のみ・会計は TWAP を par で上限クランプ。`twapDuration` immutable＋deploy 時 `getOracleState` readiness。下限強制なし→ `REMEDIATION_PROPOSALS` OR2。 |
...
## Part B（是正案・`REMEDIATION_PROPOSALS.md`・マージ禁止）

新規 HIGH/MEDIUM 実バグは無し。以下は LOW/informational（資金喪失リスクなし・任意）：

1. **RD5**（LOW）：`_deposit`/`_mint` に任意 `require(shares > 0)`（zero-share dust 拒否）。
2. **AC8-obs**（LOW）：`setPerformanceFee`/`setManagementFee`＋registry gov 移転に event 追加。
3. **OR2/⑤**（LOW・継続）：Pendle `twapDuration_ >= 900`（≥15分 TWAP）下限強制。
4. **⑥**（info・継続）：`performanceFee` dead-code を not-implemented revert 化 or 配線。
5. **AC4**（ops・再確認）：本番 mainnet は Timelock(48h)＋2-of-3 Safe 必須（既存 deploy gate の再掲）。

> いずれも **behavior 変更を伴うため凍結コード(`68eb3ec`)に入れず**、SHIN 承認後にハードニング最終形へ織り込み→再凍結して外部提出。

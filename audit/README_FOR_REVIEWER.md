...
6. **【レビュアー宛 open question — 内部 Threat Council 2026-07-11 由来】** 詳細＝`audit/THREAT_COUNCIL_2026-07-11.md`。
   - **① ストレス時の退出/移行 liveness（HIGH・要見解）**：`received >= mark` の hard-require（M13-16）により realizable<mark（depeg/slippage 超）で **出金・detach・`setAdapter(0)` が全 revert**。`setEmergencyShutdown` は `withdraw` revert には耐えるが `adapter.totalAssets()` revert で brick（try/catch 外）。PoC＝`test/StressExitFreeze.t.sol`（4本 green）。想定修正＝force-detach（try/catch）＋Ethena governance slippage setter＋totalAssets try/catch 化（workspace ADR-007・未実装）。**この設計トレードオフ（資金 stranding 防止 vs stress 退出性）へのレビュアー見解を求む。**

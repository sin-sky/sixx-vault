...
`THREAT_COUNCIL_REMAINING_2026-07-11.md` の合議で **新規 HIGH/MEDIUM 実バグは検出されず**（②③④⑦⑧ 全ベクター安全確認済 or 既知運用/将来スコープ）。新規 PoC 28本＝`test/ThreatCouncilRemaining.t.sol`（非フォーク全 182 本 green）。レビュアーに見解を求めたい点：

1. **RD5 zero-share dust（LOW）**：OZ v5 ERC-4626 は `shares>0` 未強制＝極小入金が dust を取り 0 share を mint（自己負担・第三者搾取不能・非 insolvency）。オンチェーン `require(shares>0)`（`REMEDIATION_PROPOSALS` P1）を課すべきか、フロント min-deposit 受容で十分か。
2. **AC4 registry 信頼前提**：gov（本番=Timelock+2-of-3 Safe）は registry 登録済 adapter へのみルート可能だが、register 権限で悪意 adapter 登録→切替は理論上可能（Timelock 48h が検知窓）。この信頼境界の許容度への見解。
3. **OR2 Pendle twapDuration 下限なし**（immutable＋deploy 時 readiness で緩和）。`>= 900`（15分）下限強制（`REMEDIATION_PROPOSALS` P3）は必要か。
4. **AC8 event GAP**：`setPerformanceFee`/`setManagementFee`＋registry gov 移転が event 未発火（可観測性のみ・資金無関係）。
5. **⑧ 将来 permit-forwarder**：現 vault は署名面ゼロ（cross-chain replay 対象なし）。④ で permit-forwarder を足す際、EIP-712 domain に chainId 必須（本書＋合議に設計注記済）。
